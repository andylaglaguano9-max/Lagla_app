...
<body>
  <div class="container">
    ...
    <div class="card">
      <form method="POST" action="juegos_actualizar.php">
        <input type="hidden" name="juegoId" value="<?= (int)$juego['JuegoId'] ?>">
        <div class="row">
          <div>
            <label>Nombre</label>
            <input name="nombre" value="<?= htmlspecialchars((string)$juego['Nombre']) ?>" required>
          </div>
          <div>
            <label>Precio</label>
            <input type="number" step="0.01" name="precio" value="<?= htmlspecialchars((string)$juego['Precio']) ?>" required>
          </div>
        </div>
        <div class="row">
          <div>
            <label>PlataformaId</label>
            <input type="number" name="plataformaId" value="<?= htmlspecialchars((string)$juego['PlataformaId']) ?>" required>
          </div>
          <div>
            <label>Género</label>
            <input name="genero" value="<?= htmlspecialchars((string)$juego['Genero']) ?>" required>
          </div>
        </div>
        <label>Imagen URL</label>
        <input name="imagenUrl" value="<?= htmlspecialchars((string)($juego['ImagenUrl'] ?? '')) ?>">
        <label>Descripción</label>
        <textarea name="descripcion"><?= htmlspecialchars((string)($juego['Descripcion'] ?? '')) ?></textarea>
        <label><input type="checkbox" name="estado" <?= ((int)($juego['Estado'] ?? 1)) === 1 ? 'checked' : '' ?>> Activo</label>
        <div class="actions">
